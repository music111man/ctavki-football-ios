    ...
    func loadModels() -> Single<[ForecastViewModel]> {
        ...
        return activeBetsQ
            .flatMap {[weak self] activeBets -> Observable<[ForecastViewModel]> in
                ...
                return Observable.combineLatest(activeBets.map {self.load(betId: $0.id, activeBets: activeBets)})
                    .map { models -> [ForecastViewModel] in
                        models.compactMap { $0 }
                    }
            }.asSingle()
    }
    
    private func load(betId: Int, activeBets: [Bet]) -> Observable<ForecastViewModel?> {
        guard let bet = activeBets.first(where: {$0.id == betId}), let typeId = bet.typeId else {
            return .just(nil)
        }
            ...
            .flatMap { (bet, activeBets, betType, teams) -> Observable<ForecastViewModel?> in
                ...
                .flatMap { (res1, res2) -> Observable<ForecastViewModel?> in
                    ...
                    if let typeArg = bet.typeArg {
                        typeTitle =  betType.longTitle.replace("%x%", with: "\(typeArg)")
                    } else {
                        typeTitle = betType.longTitle
                    }
                    var seria1 = 0
                    for bet in bets1 {
...
//                            printAppEvent("typeArg = \(typeArg) typeId=\(betType.id)")
//                            if [34, 38].contains(betType.id) {
//                                typeArg += 0.5
//                            } else {
//                                typeArg -= 0.5
//                            }
//                            printAppEvent("typeArg = \(typeArg)")
